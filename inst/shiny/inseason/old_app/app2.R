...
show_keyring_modal <- function() {
  showModal(modalDialog(
    ...
    footer = tagList(
      modalButton("Cancel"),
      actionButton("kr_save", "Save to keyring", class = "btn-primary")
    ),
    easyClose = FALSE
  ))
}
...
# only assert if present (helps during development)
for (nm in c("db_connect", "get_catch_data_date", "get_council_catch_data", "get_length_freq_data_date")) {
  if (exists(nm, mode = "function", inherits = TRUE)) assert_fun(nm)
}
...
observeEvent(input$kr_save, {
  req(requireNamespace("keyring", quietly = TRUE))
  ...
  tryCatch({
    ...
    showNotification(paste("Failed to save to keyring:", conditionMessage(e)),
                     type = "error", duration = 10)
    return()
  })

  removeModal()
  showNotification("Saved credentials to keyring.", type = "message", duration = 4)
})


  # Timer banner driven by invalidateLater
  ...
  output$status <- renderText({
    ...
    paste0(
      ...
      "CPUE data: ", if (is.null(cpue)) "not pulled" else paste0("pulled (rows: ", nrow(cpue), ")")
    )
  })

  # Add near the top with other libraries:
# library(lubridate)


observeEvent(input$pull_data, {

  # ---- keyring pre-check (before db_connect) ----
  if (!requireNamespace("keyring", quietly = TRUE)) {
    showNotification("Package 'keyring' is not installed. Install it to use saved credentials.",
                     type = "error", duration = 10)
    return()
  }
  ...
  gr       <- input$gear
  prop_m   <- input$prop_min
  ublend   <- input$use_blend
  month_gf <- input$month_gear_facet

  dmin <- parse_mdy(dmin_chr)
  dmax <- parse_mdy(dmax_chr)

  if (is.na(dmin) || is.na(dmax)) {
    showNotification("Date parsing failed. Use mm/dd/yyyy (e.g., 01/15/2026).",
                     type = "error", duration = 10)
    return()
  }

  # 10-yr back window for CPUE/council
  dmin2 <- lubridate::as_date(dmax) %m-% lubridate::years(10)
  ...
  withProgress(message = "Pulling data…", value = 0, {
    ...
    if (is.null(con_rv())) {
      ...
      if (inherits(con, "error") || is.null(con)) {
        showNotification(paste("DB connect failed:", conditionMessage(con)),
                         type = "error", duration = 10)
        con_rv(NULL)
        return()
      }
    ...
    if (inherits(cat_dat, "error") || is.null(cat_dat)) {
      showNotification(paste("Catch pull failed:", conditionMessage(cat_dat)),
                       type = "error", duration = 10)
      catch_rv(NULL)
    } else {
      catch_rv(cat_dat)
    ...
    if (inherits(cc_dat, "error") || is.null(cc_dat)) {
      showNotification(paste("Council pull failed:", conditionMessage(cc_dat)),
                       type = "error", duration = 10)
      council_rv(NULL)
    } else {
    ...
    if (inherits(lf_dat, "error") || is.null(lf_dat)) {
      showNotification(paste("Length pull failed:", conditionMessage(lf_dat)),
                       type = "error", duration = 10)
      lf_rv(NULL)
    } else {
      lf_rv(lf_dat)
    ...
    if (inherits(cpue_dat, "error") || is.null(cpue_dat)) {
      showNotification(paste("CPUE pull failed:", conditionMessage(cpue_dat)),
                       type = "error", duration = 10)
      cpue_rv(NULL)
    } else {
      cpue_rv(cpue_dat)
  ...
  output$p_cpue_wt <- renderPlot({
    ...
    out <- plot_observer_cpue(cpue_dat, plot_type = input$cpue_plot_type, month_gear_facet =input$month_gear_facet)
