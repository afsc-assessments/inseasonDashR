...
# numeric (unquoted) collapse
collapse_filters_num <- function(x) {
  paste(x, collapse = ",")
}

# same as sql_filter() but does NOT quote x (for numeric comparisons)
...
keyring_ready <- function(service, username = NULL) {
  ...
  # If username not provided, accept "any credential exists for this service"
...
show_keyring_modal <- function(session) {
  showModal(modalDialog(
    ...
    tags$hr(),
    p(tags$small("Passwords are stored in your system keychain/credential store via keyring; they are not saved in the app.")),
